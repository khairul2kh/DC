...
public class HibernateRadiologyDAO implements RadiologyDAO {
    ...
    public OrderPacs saveOrderPacs(OrderPacs pacs) {
        return (OrderPacs) sessionFactory.getCurrentSession().merge(pacs);
    }
    ...
    public HisOrder saveHisOrder(HisOrder hisOrder) {
        return (HisOrder) sessionFactory.getCurrentSession().merge(hisOrder);
    }

    public PacsData savePacsData(PacsData pacsData) {
        return (PacsData) sessionFactory.getCurrentSession().merge(pacsData);
    }

    public HisReport saveHisReport(HisReport hisReport) {
        return (HisReport) sessionFactory.getCurrentSession().merge(hisReport);
    }

    public List<RadiologyTest> getCompletedRTests(Date date, String status, Set<Concept> concepts, List<Patient> patients, int page, int pageSize) throws ParseException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(
                RadiologyTest.class);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String startDate = sdf.format(date) + " 00:00:00";
