...
// For all Doc and CH Group by Doc and CH Id
    ...
    public List<DiaPatientServiceBill> getCommissionCal(int docId, Date sDate, Date eDate) throws DAOException {
        ...
        if (docId == 1) {
            hql = "from  DiaPatientServiceBill d "
                    ...
                    + "' AND '"
                    + endDate
                    + "' AND d.comStatus=false AND d.refRmpId<1 AND d.voided=false GROUP BY d.refDocId order by d.refDocId asc";
        }
        if (docId == 2) {
            hql = "from  DiaPatientServiceBill d "
                    + " where d.refRmpId>='"
                    ...
                    + "' AND '"
                    + endDate
                    + "' AND d.comStatus=false AND d.refRmpId>0 AND d.voided=false GROUP BY d.refRmpId order by d.refRmpId asc";
    ...
    public List<DiaCommissionCal> getDiaComCal(int docId, Date sDate, Date eDate) throws DAOException {
        ...
        hql = "from  DiaCommissionCal d "
                ...
                + "' AND '"
                + endDate
                + "' AND d.status=false AND d.refRmpId<1 AND d.diaPatientServiceBill.voided=false ";
    ...
    public List<DiaCommissionCal> getDiaComCalRmp(int rmpId, Date sDate, Date eDate) throws DAOException {
        ...
        hql = "from  DiaCommissionCal d "
                ...
                + "' AND '"
                + endDate
                + "' AND d.status=false AND d.refRmpId>0 AND d.diaPatientServiceBill.voided=false";
    ...
    public List<DiaCommissionCal> getDiaComCalRmpPay(int rmpId, Date sDate, Date eDate) throws DAOException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String startDate = sdf.format(sDate) + " 00:00:00";
        String endDate = sdf.format(eDate) + " 23:59:59";
        String hql = null;
    ...
    public List<DiaCommissionCal> listDiaComCal(int dc, Date sDate, Date eDate) throws DAOException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String startDate = sdf.format(sDate) + " 00:00:00";
        String endDate = sdf.format(eDate) + " 23:59:59";
        String hql = null;
        ...
        if (dc == 1) {
            hql = "from  DiaCommissionCal d "
                    ...
                    + "' AND '"
                    + endDate
                    //+ "' AND d.status=false AND d.refRmpId<1 GROUP BY diaPatientServiceBill order by d.refId asc";
                    + "' AND d.status=false AND d.refRmpId<1 GROUP BY d.refId order by d.refId asc";
        }
        if (dc == 2) {
            hql = "from  DiaCommissionCal d "
                    //  + " where d.refRmpId>='"
    ...
    public List<DiaCommissionCal> getDiaComListById(int id) throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(DiaCommissionCal.class);
        criteria.add(Restrictions.eq("id", id));
    ...
    public DiaCommissionCal getDiaComByBillId(int billId) throws DAOException {
        Criteria criteria = sessionFactory.getCurrentSession().createCriteria(DiaCommissionCal.class);
        criteria.add(Restrictions.eq("diaPatientServiceBill.billId", billId));
        return (DiaCommissionCal) criteria.uniqueResult();
    }
    ...
    public List<DiaCommissionCal> listDiaComCalGroupByBillId(int docId, int docch, Date sDate, Date eDate) throws DAOException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String startDate = sdf.format(sDate) + " 00:00:00";
        String endDate = sdf.format(eDate) + " 23:59:59";
        String hql = null;
        ...
        if (docch == 1) {
            hql = "from  DiaCommissionCal d "
                    ...
                    + "' AND '"
                    + endDate
                    + "' AND d.status=false AND d.refRmpId<1 AND d.diaPatientServiceBill.voided=false GROUP BY d.diaPatientServiceBill.billId order by d.diaPatientServiceBill.billId";
        }
        if (docch == 2) {
            hql = "from  DiaCommissionCal d "
                    //  + "INNER JOIN DiaPatientServiceBillItem di ON d.billId = di.diaPatientServiceBill.billId "
                    ...
                    + "' AND '"
                    + endDate
                    + "' AND d.status=false AND d.refRmpId>0 AND d.diaPatientServiceBill.voided=false GROUP BY d.diaPatientServiceBill.billId order by d.diaPatientServiceBill.billId";
        }
