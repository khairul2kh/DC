    ...
    <tbody>
        <c:forEach var="test" items="${tests}" varStatus="index">
            ...
            <tr class="${klass}">
                ...
                <td>
                    <span style="font-size:16px; font-weight:bold;"> ${test.patientIdentifier} </span>
                </td>
                <td>
                    ${test.patientName}
                </td>
                    ...
                    <c:choose>
                        <c:when test="${empty test.status}">
                            <a href='javascript:getDefaultSampleId(${test.orderId});'>
                                <span style="font-size:16px; font-weight:bold;"> Accept </span>
                            </a>
                        </c:when>
                        <c:when test="${test.status eq 'accepted'}">
                            <b style="color:red;">Accepted</b>							
                        </c:when>
                    </c:choose>
                </td>
